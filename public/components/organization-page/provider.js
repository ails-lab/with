	  ...
	  self.loadAll = function () {
		  ...
		  $.when(promise).done(function (data) {
		        ...
		        if(data.page){
			          ...
			          else{self.url('http://'+data.page.url); }
			          ...
			          if(self.hero()){
			        	  $(".hero").css('background-image', 'url('+self.hero()+')');
			          }
			      }
		...
		self.moreCollections = function () {
			...
			if (self.loading() === false) {
				...
				$.ajax({
					"url": '/collection/list',
					data: "count=20&offset=" + offset + "&directlyAccessedByUserOrGroup="+JSON.stringify([{group:self.username(),rights:"READ"}]),
					"method": "get",
					"contentType": "application/json",
