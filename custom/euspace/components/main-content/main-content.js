	  ...
	  self.loadAll = function () {
		...
		self.getSpaceCollections = function () {
			...
			return $.ajax({
				...
				url: "/collection/list",
				processData: false,
				data: "loggedInUserAccess=read&offset=0&count=20&collectionHits=true&directlyAccessedByGroupName="+JSON.stringify([{group:"EUScreenXL",rights:"OWN"}]),
			}).success (function(){
			});
		};
		...
		self.moreCollections = function () {
			...
			if (self.loading() === false) {
				...
				$.ajax({
					"url": "/collection/list?loggedInUserAccess=read&count=20&offset=" + offset + "&directlyAccessedByGroupName="+JSON.stringify([{group:"EUScreenXL",rights:"OWN"}])
					"method": "get",
					"contentType": "application/json",
