...
public class ApacheHttpConnector extends HttpConnector {
	...
	private final static RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(5000)
			.setConnectionRequestTimeout(5000).setSocketTimeout(5000).build();

	public static HttpClientBuilder httpClientBuilder = HttpClientBuilder.create().setConnectionManager(connMan)
			.setConnectionManagerShared(true).setConnectionTimeToLive(1, TimeUnit.MINUTES).setMaxConnPerRoute(10)
			.setMaxConnTotal(100).setDefaultRequestConfig(requestConfig);
	...
	public FileAndType getContentAndType(String url) throws Exception {
		...
		try {
			...
			for (Header h : headers) {
				if (StringUtils.isNotEmpty(h.getValue()))
					res.mimeType = h.getValue();
			}
	...
	// }
	//
	// public <T> T postFileContent(String url, File file, String paramName,
